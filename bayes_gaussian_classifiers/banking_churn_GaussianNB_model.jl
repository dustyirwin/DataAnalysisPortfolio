...
# ╔═╡ bece81ff-3d2c-4e41-827e-12b679af45cc
# loading libraries
begin
	using CSV, DataFramesMeta, StatsBase, MLJ, MLJNaiveBayesInterface, NaiveBayes
	using StatsPlots, ColorSchemes, Measures, LaTeXStrings
	using PlutoUI, JuliaSyntax
end

# ╔═╡ 680d2a60-ca44-4804-a4a3-2ef23b7fcbd8
md"""# Banking Churn

Creating a Naive-Bayes model to classify whether a banking customer is likely to churn.
"""

# ╔═╡ 56be75dd-e8a4-497e-a227-8261ed35beac
data = CSV.File("../_data/Churn_Modelling.csv.gz") |> DataFrame

# ╔═╡ e748a5f2-019e-48e1-af85-099db1402e51
function minmaxscaler(df::DataFrame)
	_mins = []
	_maxs = []
	...
	for v in eachcol(df)
		...
		if eltype(v) <: Float64
			...
			push!(_mins, minimum(v))
			push!(_maxs, maximum(v))	
		end
	end
	...
	for (i,v) in enumerate(eachcol(df))

		if eltype(v) <: Float64

			df[!,i] = [ (x - _min) / denom for x in v ]
		end
	end
...
# ╔═╡ 0a0c8876-d16a-4a56-86c1-855b8afc409c
cleaned_data = let
	df = deepcopy(data)
	df.CreditScore = Float64.(df.CreditScore)
	
	_df = minmaxscaler(df)
	_df.loyalty = _df.Tenure ./ _df.Age
	
	select!(_df, Not([:Age,:Tenure,:RowNumber,:CustomerId,:Surname]))
	...
	coerce(_df,
		...
		:Exited => OrderedFactor,
	)
end

# ╔═╡ 1de71392-9b93-440a-abb6-e684690ffcdc
cleaned_data.Gender |> countmap

# ╔═╡ 5cd6342d-0764-40d2-a1b9-74feebf4d54f
cleaned_data.Geography |> countmap

# ╔═╡ 4a8582f6-ecae-40ba-8ba7-e7f1366c2e48
models("Bayes")

# ╔═╡ dbb415bf-7a1b-4275-8b2c-9320e827b7e1
train, test = partition(cleaned_data, 0.8, rng=42, 
	stratify=cleaned_data.Exited
)

# ╔═╡ 31839800-6149-410d-beb9-67158d273994
Xtrain, ytrain = select(train, Not(:Exited)), train.Exited

# ╔═╡ fba6b920-2dd9-4f86-b806-835cede35002
Xtest, ytest = select(test, Not(:Exited)), test.Exited

# ╔═╡ 23b63091-6a4e-40dc-81eb-5cdfa2186562
#standardizer = machine(Standardizer(), X) |> fit!

# ╔═╡ 696e51bf-7fa5-4d22-b134-4296559e6f8c
#MLJ.transform(standardizer, X)

# ╔═╡ 7a3afcf7-682b-4284-9b0a-ecdc62f81c23
_Xtrain = machine(OneHotEncoder(drop_last=true), Xtrain) |> fit! |> m -> MLJ.transform(m, Xtrain)

# ╔═╡ 4ec2aa58-f7bf-4c1c-9027-7ca3b5c74a4c
_Xtest = machine(OneHotEncoder(drop_last=true), Xtest) |> fit! |> m -> MLJ.transform(m, Xtest)

# ╔═╡ 0b765d8a-0541-492c-a4b7-4867be30e5bf
md"## Gaussian Naive-Bayes Model"

# ╔═╡ 0d68bfff-8b43-401f-ae09-6832d9df3c03
model = @load GaussianNBClassifier pkg=NaiveBayes

# ╔═╡ 60ebe02d-418f-41a7-90f9-e169caf99fd5
sel_vars = [
	:CreditScore, 
	:Geography__France, 
	:Geography__Germany, 
...
# ╔═╡ 4a93131a-a345-446e-8d5a-6a3eb0613422
_X = select(_Xtrain, sel_vars);

# ╔═╡ 72aa8564-eef3-458d-aadd-037eb2187153
models(matching(Xtrain, ytrain));

# ╔═╡ e34008bf-2f1c-4046-a6b0-e4978711a1ff
mach = machine(model(), _X, ytrain, scitype_check_level=0);

# ╔═╡ efe53f24-1849-49a5-aaca-bc97136ac634
mach |> fit!

# ╔═╡ 5e83cd1b-fae0-4c6d-a96c-c49f628c4292
p = MLJ.predict(mach, select(_Xtest, sel_vars))

# ╔═╡ 9582da7f-9d3e-46ca-9673-eff452becd19
ŷ = let
	preds = []
	
	for r in p
		i = r.prob_given_ref[0x01] > r.prob_given_ref[0x02] ? 0 : 1
		push!(preds, i)
	end
	
	preds #coerce(preds, OrderedFactor)
end

# ╔═╡ f18ed758-18f5-48ae-9962-d81102bc6f36
y = test.Exited

# ╔═╡ 38ee423e-ed71-4a7d-8527-36a0c2a28b9b
ConfusionMatrix()(ŷ,y)

# ╔═╡ f1d083fa-aa70-4aa8-8048-5a3fc9df81b0
accuracy(ŷ,y), precision(ŷ,y), recall(ŷ,y), f1score(ŷ,y), FScore(; β=1.5)(ŷ,y), area_under_curve(p,y)

# ╔═╡ 38c49a0e-4eea-409e-aeed-b3f3666cbf27
md" ## Results
The target model training metric should be chosen based on business needs depending on whether committing a Type I or Type II error is less detrimental, based on the cost of mitigating measures planned for operations."

# ╔═╡ 50ad80b5-c099-4a6a-8396-05949192586d
md"""
# Appendix A
"""

# ╔═╡ a7bbabfa-f3b5-46db-bd81-404e2795cd95
space = "<br><br><br>" |> HTML

# ╔═╡ 22e1e56a-2b72-407f-af10-9a94f5d772d3
PlutoUI.TableOfContents()

# ╔═╡ ef29fa23-c9c0-417a-a53c-97f1c577b091
...
manifest_format = "2.0"
project_hash = "78c1e111bb8716a1a9da23e5557e975d05b0077d"
