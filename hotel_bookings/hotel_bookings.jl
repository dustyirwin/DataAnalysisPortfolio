...
- Converted all NULLs & NAs into missings
...
cleaned_hotel_data = let
	df = raw_hotel_data
	# combining arrival date columns

	df.arrival_month = [ Dates.LOCALES["english"].month_value[m] for m in df.arrival_date_month ]
...
describe(cleaned_hotel_data)

# ╔═╡ 0345077b-72e3-448f-b186-a5b3f1bc9585
md"""
## Cancelations by month & hotel type
"""

# ╔═╡ aebb48a9-4bdf-4e43-b63b-4d2bfb0bb08b
hotel_data_by_month_and_type = let
	grp = groupby(cleaned_hotel_data, [:arrival_date_month, :hotel])
	df = combine(grp, 
		:is_canceled=>sum,
		:is_canceled=>length,
		:arrival_month=>unique,
		:arrival_month=>length,
	)
	
	df.is_canceled_mean = df.is_canceled_sum ./ df.arrival_month_length
	sort!(df,:arrival_month_unique)
...
let
	...
	resort_hotel_df = @subset(hotel_data_by_month_and_type, :hotel .== "Resort Hotel")
	...
	plot(
		...
		plot(
			...
			[ 
				city_hotel_df.is_canceled_sum .* city_hotel_df.is_canceled_mean,
				resort_hotel_df.is_canceled_sum .* resort_hotel_df.is_canceled_mean,
			], 
			ylabel="cancelations",
			xrotation=45,
			labels=["City"  "Resort"],
		...
		plot(
			city_hotel_df.arrival_date_month, 
			[ 
				city_hotel_df.is_canceled_mean .* 100,
				resort_hotel_df.is_canceled_mean .* 100,
			], 
			ylabel="% cancelations",
			xrotation=45,
			labels=["" ""],
...
md"**Data taken from July 2015 thru Aug 2017**"

# ╔═╡ c908c1c4-2c62-43a5-b221-8f32fb734e0d
md"""
### Observations
- The cancelations during the summer period are significantly higher than in the winter period.
- City hotel experience 10-20% more cancelations than resort hotels, depending on the season.
...
md"""
## Hotel cancelations by market segment
"""

# ╔═╡ e1bbeb7a-0c7d-4ce4-8372-6a51a88d0bef
hotel_data_by_market_segment = let
	mean_pct(x) = mean(x) * 100
	
	grp = groupby(cleaned_hotel_data, :market_segment)
	df = combine(grp, 
		:is_canceled=>sum,
		:is_canceled=>mean_pct,
		:market_segment=>length,
	)
	
	select!(df, 
		:market_segment, 
		:is_canceled_sum=>:cancelations, 
		:market_segment_length=>:bookings,
		:is_canceled_mean_pct=>:percent_canceled,
	)
...
let
	...
	scores = [ cor(cleaned_hotel_data.is_canceled, cleaned_hotel_data[!,col]) for col in correlation_cols ]
	df = DataFrame(variable=correlation_cols,score=scores)
...
md"""
### Observations
 - There is a weak positive correlation between 'lead time' and and cancelations.
 - There is a weak negative correlation between the number of special requests, required parking spaces, and booking changes.
...
# ╔═╡ 47a7c497-d5c4-4879-a6bb-49c484e163f6
