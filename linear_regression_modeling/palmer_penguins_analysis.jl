...
# ╔═╡ fb89411f-ab3c-474c-9621-f1772565eb55
# loading libraries
begin
	using CSV, DataFramesMeta, StatsBase, Dates, Distributions, HypothesisTests
...
# ╔═╡ 8233d362-5206-11ee-0f65-a98c8e3a4f16
md"# Palmer 🐧 Penguins Linear Regression Model"

# ╔═╡ 3d4a4129-a7dc-4a36-be69-c64c22cf0446
df = let
	df = CSV.File("data/penguins.csv.gz", missingstring="NA") |> DataFrame
	df = @subset(df, :species .!= "Chinstrap")
	
	select!(df, :body_mass_g, :flipper_length_mm, :bill_length_mm, :species, :sex)
	...
	df = coerce(df, 
		...
		:species => MLJ.Multiclass,
		:sex => MLJ.Multiclass,
	)
end

# ╔═╡ a43cde40-64f0-492a-8443-8f34d53b9b66
onehot = machine(OneHotEncoder(drop_last=true), df) |> fit!

# ╔═╡ 43444120-21ef-4dbc-ab1d-e564aafae445
W = MLJ.transform(onehot, df)

# ╔═╡ 13a6bcb7-1051-4c35-9473-905f4789511d
schema(W)

# ╔═╡ 9819e34e-4f34-414d-8f4c-893d4c4d76be
let
	names = [ replace(name, "sex__"=>"", "species__"=>"", "_g"=>"", "_mm"=>"") for name in names(W) ]
	cornerplot(Matrix(W), label=names, size=(900,900), xrotation=60, yrotation=0, margin=0mm)
end

# ╔═╡ 48b7167c-062d-4d64-92c8-1d53dc6e024e
target = :body_mass_g

# ╔═╡ d81eb2f7-d911-499a-ad78-1ac82c93d98f
model_choices = let
	df = models(matching(W, W.body_mass_g)) |> DataFrame
	@subset(df, :is_pure_julia .== true)
end

# ╔═╡ 2a928918-ad68-43a7-95ad-38102b575a99
model = RidgeRegressor()

# ╔═╡ b8e611c3-de5b-4f36-8bb2-2db281e6e529
mach = fit!(machine(model, select(W, Not(target)), W[!,target]), verbosity=2)

# ╔═╡ 662744f6-7050-4a6a-be47-67bb7506d4a5
fitted_params(mach)

# ╔═╡ 12add970-96b5-4579-89db-0da50e9edb7c
evaluate!(mach, measure=[rmse, mae, rsquared])

# ╔═╡ 9b0e2267-42bc-4ba9-8104-169332d64de9
preds = MLJ.predict(mach, select(W, Not(target)))

# ╔═╡ 03e4d6dd-a489-422b-89ee-44d52ef504a9
residuals = W[!,target] - preds

# ╔═╡ b3f5e5a5-64b5-489f-96e3-cc87ca6b3fb5
_r2 = rsquared(preds, W[!,target])

# ╔═╡ 5fd180d2-8025-44c4-964e-7d7bac6592be
_rmse = rmse(preds, W[!,target])

# ╔═╡ c8e6f8ee-d668-4a86-8232-2765221059f4
_mse = _rmse ^2

# ╔═╡ a0689a37-4156-408d-bddd-3ba77bec85c7
_mae = abs.(residuals) |> mean

# ╔═╡ 63d242b6-ab22-4b13-a274-326a22b48896
let
	scatter(W.bill_length_mm, W.body_mass_g, 
		legend=false, xaxis="bill_length_mm", yaxis="body_mass_g",
		smooth=true, ribbon=residuals, size=(800,500), margin=5mm,
	)
	
	annotate!(38, 6500, latexstring("y = $(round(mach.fitresult[1][1], digits=2))x + ($(round(mach.fitresult[1][2], digits=2)))"))
	annotate!(38, 6000, latexstring("r^2 = $(round(_r2, digits=2))"))
end

# ╔═╡ c89492e6-87ee-4bbc-bd98-52f24fa6ae2e
plot(1:length(preds), [df[!,target], preds], size=(900,500), label=["target" "prediction" "rmse"])

# ╔═╡ bc880c85-efac-4cd7-be9d-8bb00dc5c608
# residuals vs fitted values (preds)... if the best fit line is practically 0, we can assume homoscedacicity
scatter(preds, residuals, smooth=true)

# ╔═╡ 73a4b375-321e-4189-b5e9-ba5d0bc33515
# residuals are mostly normally distributed, this indicates the normality assumption is also valid
histogram(residuals)  

# ╔═╡ 79b4bb7a-ff01-4b27-bf55-e6ff667e3fc9
# there is some deviation from the normal distribution line in the extreme values, but mostly confirms the Normality assumption
StatsPlots.qqnorm(residuals) 

# ╔═╡ b3f748d2-f6be-48c1-938e-8a3d1ca908dc
md"""
# Appendix B
"""

# ╔═╡ cd429395-cdd5-49e9-91ad-0faed0a809ec
space = "<br><br><br>" |> HTML

# ╔═╡ 58570368-443c-4132-bfbd-dae2ada1aa5e
PlutoUI.TableOfContents()

# ╔═╡ 2fb8c8b2-c02c-4ada-ab19-d41555762962
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "ecfb05187d4750301fe3e81767f99107652f896c"
