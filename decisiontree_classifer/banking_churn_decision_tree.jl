...
begin
	...
	#using MLJBase: ConfusionMatrix
	import ScientificTypesBase, BetaML, DecisionTree, MLJDecisionTreeInterface, EvalMetrics
end

# ╔═╡ 2dac07f0-fe9f-44e9-92c2-2c92f018fbe8
md"# Banking Churn Decision Tree Model"

# ╔═╡ c19202d0-1139-4926-b71f-5cac0e4002eb
df = let
	df = CSV.File("../_data/Churn_Modelling.csv.gz", normalizenames=true) |> DataFrame
	df.LoyaltyScore = df.Tenure ./ df.Age
	select!(df, :Exited, Not([:Exited, :RowNumber, :Surname, :CustomerId, :Tenure, :Age]))
	#df.DebtToIncomeRatio = df.Balance ./ df.EstimatedSalary
	#df.Exited = string.(df.Exited)
...
train, test = MLJ.partition(df, 0.80, stratify=df.Exited);

# ╔═╡ 4efd5298-e726-431d-887f-39b20f3e4bf6
# target variable is unbalanced, stratifying train and test sets on 'Exited'
countmap(test.Exited)

# ╔═╡ 9e17a31f-c584-4cea-abd9-a5e51ed18321
models(matching(select(train,Not(:Exited)),train.Exited)) |> DataFrame

# ╔═╡ f715dfa8-ccd2-447f-bc47-ae5b0a48e2b0
train.Exited

# ╔═╡ 978f6dd8-cac6-402e-888a-3bcfa31e38a7
onehot_enc = machine(MLJ.OneHotEncoder(drop_last=true), select(df, Not(:Exited))) |> MLJ.fit!

# ╔═╡ 03a2b4a4-7be3-406b-addd-0c6f335f81da
Xtrain, ytrain = MLJ.transform(onehot_enc, select(train, Not(:Exited))), train.Exited;

# ╔═╡ d9fc8181-8271-4c39-8282-0ab3c5163e0e
Xtest, ytest = MLJ.transform(onehot_enc, select(test, Not(:Exited))), test.Exited;

# ╔═╡ 290d3b6b-eaa9-4930-8a2b-9bf314084c27
schema(train)

# ╔═╡ fd27c652-623f-458a-8323-dc50cafda5b5
md"## Model Instantiation"

# ╔═╡ 68a57689-dba0-4f80-99ab-58d024686a44
tree = let
	t = @load DecisionTreeClassifier pkg=DecisionTree  verbosity=0
	t(
	  #max_depth = -1, 
	  #min_samples_leaf = 5, 
	  #min_samples_split = 2, 
...
r1 = range(tree, :min_samples_leaf, lower=2, upper=50, scale=:log10)

# ╔═╡ cf411ad6-f79f-4faa-a5cb-8125078eb3bc
r2 = range(tree, :min_purity_increase, lower=0.001, upper=1.0, scale=:log10)

# ╔═╡ b40d87d1-d41f-47d2-89eb-a4f2763531a6
r3 = range(tree, :max_depth, lower=4, upper=40, scale=:log10)
...
self_tuning_tree = TunedModel(
	...
	range=[r1,r2,r3],
	measure=recall, # the cost of a false negative is high, tuning for best recall score
	acceleration=CPUThreads(),
)

# ╔═╡ da2d78b6-545b-4e78-98ae-235e423ddd00
mach = machine(self_tuning_tree, Xtrain, coerce(ytrain, OrderedFactor))

# ╔═╡ 2edaa88c-1a8b-448e-99ef-ddd50b6232e6
MLJ.fit!(mach, verbosity=0)

# ╔═╡ b165b677-30fd-47e1-8e96-27fea557b8ab
# ╠═╡ disabled = true
#=╠═╡
mach = machine("models/dt_banking_churn.jls")
  ╠═╡ =#

# ╔═╡ 9f867f9b-48a4-4499-8103-e6412adc1c6e
md"## Model Metrics"

# ╔═╡ 56cebe0f-fbc4-4da2-8dee-39d487239637
report(mach).best_model

# ╔═╡ 7231c9d3-b59c-4fc4-b1ff-b511fde59447
mach.report

# ╔═╡ a95f7bcc-aa61-441f-9bdf-d2f2c45a4ba7
report(mach).best_report.print_tree(3)

# ╔═╡ d6e2f9df-74a5-48e7-946b-88452492fa2d
plot(mach)

# ╔═╡ d1255aea-dc2c-4764-906d-cef27686afad
ŷ = [ i.prob_given_ref[2] > i.prob_given_ref[1] ? 1 : 0 for i in MLJ.predict(mach, Xtest) ]

# ╔═╡ 390cac41-7ad2-4bdd-b4d7-62ef3a4b1f85
y = [ y == 1 ? 1 : 0 for y in ytest ]

# ╔═╡ 4345ae8a-7a06-4aaa-9686-bc92ff492b7f
balanced_accuracy(ŷ,y), accuracy(ŷ,y), f1score(ŷ,y), precision(ŷ,y), recall(ŷ,y) 

# ╔═╡ 513ae34d-57fc-40b8-82b5-81c10f470bb6
confmat(ŷ,y)

# ╔═╡ 667174ac-e3c3-4f7c-84b9-160fe97896d7
# mean $ loss risk of churned customer
@subset(df, :Exited .== 1).Balance |> mean

# ╔═╡ f36ced51-ea60-4780-8cd4-12de36c34b80
md"## Model Persistance"

# ╔═╡ 1c3e35fa-327f-4d36-80bb-8474f14fae1f
# ╠═╡ disabled = true
#=╠═╡
MLJ.save("models/dt_banking_churn.jls", mach)
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "1ed364b9be19eb38715ef17fe9973d1c4222433e"
...
git-tree-sha1 = "649aa716c3e0decc04732c607648fb73e23ddc4b"
