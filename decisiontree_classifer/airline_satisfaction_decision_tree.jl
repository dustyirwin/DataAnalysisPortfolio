...
# ╔═╡ a9eea634-ca7d-44ce-bb96-52aa5186189b
# loading libraries
begin
	using CSV, DataFramesMeta, StatsBase, MLJ
	using StatsPlots, ColorSchemes, Measures, LaTeXStrings
	using PlutoUI, JuliaSyntax, Random, Logging
	import BetaML, EvalMetrics, DecisionTrees, MLJDecisionTreesInterface, MLJFlux
end

# ╔═╡ 2dac07f0-fe9f-44e9-92c2-2c92f018fbe8
md"""# Invisto Airline Satisfaction Decision Tree Model

In this notebook we will create a Decision Tree model to predict customer satisfaction based on survey results from 'Invisto Airline'.
"""

# ╔═╡ fd27c652-623f-458a-8323-dc50cafda5b5
md"## Decision Tree Model"

# ╔═╡ 68a57689-dba0-4f80-99ab-58d024686a44
tree = let
	t = @load DecisionTreeClassifier pkg=BetaML  verbosity=1
	t(
	  max_depth = 30, 
	  min_records = 2, 
	  #min_samples_leaf = 1, 
...
# ╔═╡ 1061cd32-5495-4bbb-9a0f-cebc638682f2
md"## Model Tuning"

# ╔═╡ 21c0a1b1-531c-4dbc-8b42-0a3736e9deb3
r1 = range(tree, :min_records, lower=2, upper=50, scale=:log10)

# ╔═╡ cf411ad6-f79f-4faa-a5cb-8125078eb3bc
r2 = range(tree, :min_gain, lower=0.001, upper=1.0, scale=:log10)

# ╔═╡ b40d87d1-d41f-47d2-89eb-a4f2763531a6
r3 = range(tree, :max_depth, lower=2, upper=30, scale=:log10)

# ╔═╡ b25acff3-7d9d-4800-bfc3-0ae5c078a6f2
...
# ╔═╡ 320479d9-e445-4326-9004-1c097c21e4cc
# ╠═╡ disabled = true
#=╠═╡
mach = machine("models/dt_aireline_satisfaction.jls")
  ╠═╡ =#

# ╔═╡ 9f867f9b-48a4-4499-8103-e6412adc1c6e
md"## Model Metrics"

# ╔═╡ 960eb109-8cf2-4d1b-b1d6-37a574a80c6e
md""" ## Results

The model shows a moderate accuracy of 0.65, excellent recall at over 0.99 and high f1 score of 0.75. The model would serve well to detect customer **dissastisfaction**, but less so for customer satisfaction. Evaluation of this model could be used to identify variables that results in a low satisfaction rating.   

"""

# ╔═╡ f36ced51-ea60-4780-8cd4-12de36c34b80
md"## Model Persistance"

# ╔═╡ bfeb3013-7e55-47ff-85b1-14da5a3165b0
# ╠═╡ disabled = true
#=╠═╡
MLJ.save("models/dt_airline_satisfaction.jls", mach)
  ╠═╡ =#

# ╔═╡ 89cf5d28-046d-4ea8-b593-a28c023a8e96
md"""
# Appendix A - Data
"""

# ╔═╡ c19202d0-1139-4926-b71f-5cac0e4002eb
df = let
	df = CSV.File("../_data/Invistico_Airline.csv.gz", normalizenames=true) |> DataFrame
	
	dropmissing!(df)
	
	df.Departure_Delay_in_Minutes = Float64.(df.Departure_Delay_in_Minutes)
	df.satisfaction = [ i == "satisfied" ? 1 : 0 for i in df.satisfaction ]
	
	coerce(df, 
		:satisfaction => Binary,
		:Customer_Type => Binary,
		:Type_of_Travel => Binary,
		:Seat_comfort => OrderedFactor,
		:Departure_Arrival_time_convenient => OrderedFactor,
		:Food_and_drink => OrderedFactor,
		:Gate_location => OrderedFactor,
		:Inflight_wifi_service => OrderedFactor,
		:Inflight_entertainment => OrderedFactor,
		:Online_support => OrderedFactor,		
		:Ease_of_Online_booking => OrderedFactor,
		:On_board_service => OrderedFactor,
		:Leg_room_service => OrderedFactor,
		:Baggage_handling => OrderedFactor,
		:Checkin_service => OrderedFactor,
		:Cleanliness => OrderedFactor,
		:Online_boarding => OrderedFactor,
		:Class => Multiclass,
		:Age => Continuous,
		:Flight_Distance => Continuous,
	)
end

# ╔═╡ 2c292211-d541-48a3-a0d8-cf230763d2f7
countmap(df.satisfaction), countmap(df.Type_of_Travel)

# ╔═╡ 3a6ea4e8-d5bd-460d-a2ca-0ffa246e3eff
train, test = MLJ.partition(df, 0.75, stratify=df.satisfaction)

# ╔═╡ 636b2873-76a8-4cbc-9c50-829986eadae7
strain, stest = sample(eachrow(train), 25_000, replace=false) |> DataFrame, 
				sample(eachrow(test), 5_000, replace=false) |> DataFrame

# ╔═╡ c39b92fc-4c59-4589-9066-1d4cb9b3d866
mach = machine(self_tuning_tree, select(strain, Not(:satisfaction)), strain.satisfaction)

# ╔═╡ cf1d1492-5b75-4973-81dc-4510209e0c3f
MLJ.fit!(mach, verbosity=2)

# ╔═╡ bebf96cd-00ac-4e30-9e49-f8c0b2366f5b
plot(mach)

# ╔═╡ ced5ab68-eb8d-44ef-b28a-200aa9e93f76
report(mach).best_model

# ╔═╡ aa846682-3278-4ed0-bd44-ce4bf2af7cdd
y, ŷ = begin
	stest.satisfaction,
	[ i.prob_given_ref[2] > i.prob_given_ref[1] ? 1 : 0 for i in MLJ.predict(mach, select(stest,Not(:satisfaction))) ]
end

# ╔═╡ 31186d47-a742-4b52-b770-cb097f27a129
# ╠═╡ show_logs = false
confusion_matrix(ŷ,y)

# ╔═╡ 8d4eaf1e-c416-4849-99ba-faa4943553d5
# ╠═╡ show_logs = false
accuracy(ŷ,y), precision(ŷ,y), recall(ŷ,y), f1score(ŷ,y)

# ╔═╡ 4efd5298-e726-431d-887f-39b20f3e4bf6
# target variable is unbalanced, stratifying train and test sets on 'Exited'
countmap(test.satisfaction)

# ╔═╡ 9e17a31f-c584-4cea-abd9-a5e51ed18321
models(matching(select(train,Not(:satisfaction)),train.satisfaction)) |> DataFrame

# ╔═╡ 290d3b6b-eaa9-4930-8a2b-9bf314084c27
schema(train)

# ╔═╡ d58a290b-6567-4f61-a34e-2aaa99488a98
md"""
# Appendix B
"""

# ╔═╡ 5a783c72-516d-4a33-ae5a-d9bc967afcc4
PlutoUI.TableOfContents()

# ╔═╡ b0cb8272-e4e0-46e4-a5af-8f8fba9b5cfc
...
# ╔═╡ eebdc088-cef1-4d69-97ee-3b8834fee637
space = "<br><br><br>" |> HTML
...
manifest_format = "2.0"
project_hash = "c11335b116a240448a9e6be3d0da1eaec32d62e2"
